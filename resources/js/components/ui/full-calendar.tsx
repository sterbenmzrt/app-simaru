...
const CalendarDayView = () => {
    ...
    const handleHourClick = (hourDate: Date) => {
        const event = events.find(
            (e) => isSameHour(e.start, hourDate));

            if (event) {
                onEventClick?.(event);
            } else {
                console.log('Clicked empty hour slot:', hourDate);
            }
    };
    ...
    return (
        ...
        <div className="flex-1">
            {hours.map((hour) => (
            <div key={hour.toString()} onClick={() => handleHourClick(hour)}>
...
//     return (
//         <div className="relative flex h-full overflow-auto pt-2" onClick={() => onEventClick?.(getHours(hours[0]))}>
//             <TimeTable />
//             <div className="flex-1">
//                 {hours.map((hour) => (
                    ...
                    {weekDates.map((hours, i) => {
                        return (
                            <div
                                ...
                                key={hours[0].toString()}onClick={() => onEventClick?.(events[i])}
                            >
                                {hours.map((hour) => (
                                    <EventGroup key={hour.toString()} hour={hour} events={events} />
                                ))}
                    ...
                    return (
                        ...
                        >
                            ...
                            {currentEvents.map((event) => {
                                return (
                                    <div key={event.id} className="flex items-center gap-1 rounded px-1 text-sm" onClick={() => onEventClick?.(event)}>
